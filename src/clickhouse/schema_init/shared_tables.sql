-- ClickHouse Multi-Tenant Shared Tables Schema (Rebuilt from Canonical Mappings)
-- Built from canonical mapping files as source of truth
-- Corrected SCD Type configurations: tickets=Type 2, all others=Type 1

-- =============================================================================
-- COMPANIES TABLE (SCD Type 1)
-- Built from mappings/canonical/companies.json
-- =============================================================================
CREATE TABLE IF NOT EXISTS companies (
    ...
    -- Business fields from canonical mapping
    company_name String,
    ...
    invoice_template_id Nullable(String),
    price_schedule_id Nullable(String),
    currency_id Nullable(String),
    currency_symbol Nullable(String),
    company_entity_type Nullable(String),
    ...
    -- Required ClickHouse metadata fields
...
ENGINE = MergeTree()
ORDER BY (tenant_id, id, updated_date)
SETTINGS index_granularity = 8192;

-- =============================================================================
-- CONTACTS TABLE (SCD Type 1)
-- Built from mappings/canonical/contacts.json
-- =============================================================================
...
-- =============================================================================
-- TICKETS TABLE (SCD Type 2)
-- Built from mappings/canonical/tickets.json
-- =============================================================================
CREATE TABLE IF NOT EXISTS tickets (
    ...
    -- SCD Type 2 fields (tickets only)
...
ENGINE = MergeTree()
ORDER BY (tenant_id, id, effective_start_date)
SETTINGS index_granularity = 8192;

-- =============================================================================
-- TIME ENTRIES TABLE (SCD Type 1)
-- Built from mappings/canonical/time_entries.json
-- =============================================================================
...
ALTER TABLE companies ADD INDEX idx_company_source (source_table) TYPE set(10) GRANULARITY 1;
...
ALTER TABLE contacts ADD INDEX idx_contact_source (source_table) TYPE set(10) GRANULARITY 1;
...
ALTER TABLE tickets ADD INDEX idx_ticket_current (is_current) TYPE set(2) GRANULARITY 1;
ALTER TABLE tickets ADD INDEX idx_ticket_source (source_table) TYPE set(10) GRANULARITY 1;
...
ALTER TABLE time_entries ADD INDEX idx_time_source (source_table) TYPE set(10) GRANULARITY 1;

-- =============================================================================
-- MATERIALIZED VIEWS FOR ANALYTICS (Optimized)
-- =============================================================================
...
AS SELECT
    ...
    id as company_id,
    company_name,
    toDate(updated_date) as date,
    count() as total_records,
    count() as current_records
FROM companies
GROUP BY tenant_id, company_id, company_name, date;

-- Ticket metrics view (SCD Type 2 aware)
...
GROUP BY tenant_id, company_id, status, date;

-- =============================================================================
-- TENANT ISOLATION FUNCTIONS
-- =============================================================================

-- Function to validate tenant access (to be used in application layer)
-- This would be implemented in the Node.js API layer for row-level security